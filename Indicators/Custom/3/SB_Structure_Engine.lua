...
local function updateBoxesAndBis(period, canRender)
    ...
    local londonStartMin = normalizeMinute(instance.parameters.londonstartmin or (1 * 60))
    local londonEndMin = normalizeMinute(instance.parameters.londonendmin or (5 * 60))
...
local function render(period, canRender, mode)
    ...
    if not canRender then
        ...
        if instance.parameters.debug then
            safeTextSet(O.txtDebug, period, src.low[period] - offset * 2, "GATE: WAIT_UPSTREAM_TRADE_DAY")
        end
        return
    end
...
function Init()
    ...
    p:addBoolean("showbis1", "Enable BIS1 (Prev 22:00)", "", true)
    ...
    p:addInteger("londonstartmin", "London Box Start Minute", "", 1 * 60)
    p:addInteger("londonendmin", "London Box End Minute", "", 5 * 60)
